 ...
 * 加载CSV文件并相应地初始化UI元素
 *
 * @param {string[]} settings -Literal['uni', 'multi'] 单变量/多变量数据集
 *
 * 对于每个数据集，该函数会:
 * - 获取对应的CSV文件(例如 `./uni.csv`)
 * - 选中第一个评分复选框
 * - 通过调用`phraseInputTable`用解析的数据填充输入表格
 * - 切换诸如"指标"、"类型"和"时间跨度"等类别的设置
 * - 调用`toggleSelectAll`来设置评分选项
 */
  ...
  settings.forEach((setting) => {
    fetch(`./${setting}.CSV`)
      ...
      .then((response) => {
        if (!response.ok) {
          throw new Error(
            `Cannot fetch file ./${setting}.CSV or ./${setting}.csv`
          );
        }
      ...
      .then((text) =>
        Papa.parse(text, {
          ...
          complete: (results) => {
            ...
            toggleCategory("Strategy", setting, true, false);
            // 设置评分选项
...
function phraseInputTable(input, setting) {
  ...
  const keys = Object.keys(methods).filter(
    (item) => item !== "Dataset-Quantity-metrics"
  );
  // 存入all_data
  keys.forEach((name) => {
  ...
  for (let i = 8; i < input.length; i++) {
    ...
    const [dataset, metric] = key.split("-96-");

    if (!allData[setting].dataset.includes(dataset)) {
      allData[setting].dataset.push(dataset);
    }
  ...
  sortedCategories.forEach((category) => {
    ...
    categoryLabel.appendChild(
      document.createTextNode(` ${category.replace("_", " ")}`)
    );
...
function submitSelection(setting) {
  ...
  selectDatasets.forEach((dataset) => {
    selectMetrics.forEach((metric) => {
      ...
      const sortedMethods = selectedMethods.sort(
      ...
      if (i < sortedMethods.length - 1) rank[sortedMethods[i]].rank2 += 1;
      if (i < sortedMethods.length - 2) rank[sortedMethods[i + 1]].rank3 += 1;
    });
  });
  ...
  Object.keys(rank).forEach((method) => {
    if (selectScore === "3") {
      const weight1 =
        document.getElementById(`score-${setting}/3/1`).value || 0;
      const weight2 =
        document.getElementById(`score-${setting}/3/2`).value || 0;
      const weight3 =
        document.getElementById(`score-${setting}/3/3`).value || 0;
      rank[method].score =
        rank[method].rank1 * weight1 +
        rank[method].rank2 * weight2 +
        rank[method].rank3 * weight3;
    } else if (selectScore === "2") {
      rank[method].score =
        rank[method].rank1 + rank[method].rank2 + rank[method].rank3;
    } else {
